...
public class CarSpawner : MonoBehaviour
{
   ...
   [SerializeField] GameObject carPrefab0;
   
   [SerializeField] TerrainBlock terrain;
   [SerializeField] float minSpawnDuration = 3;
   [SerializeField] float maxSpawnDuration = 9;
   ...
   private void Update()
   {
        ...
        var spawnPos = this.transform.position + Vector3.right*
        (isRight ? -(terrain.Extent + 1) : terrain.Extent +1);

    GameObject[] Cars= new GameObject []
    {
        carPrefab, carPrefab0
    };

    int RandomValue = Random.Range(0 , 2);

    var go = Instantiate(
        original : Cars[RandomValue],
        position : spawnPos,
